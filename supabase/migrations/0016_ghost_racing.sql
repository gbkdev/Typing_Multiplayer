-- 0016_ghost_racing.sql
-- Lets a player race a "ghost" of one of their own past runs. We store the
-- exact text typed and a time-series of progress so the ghost's position
-- can be replayed and compared against live, using the identical text.

alter table public.practice_results
  add column text_snapshot text,
  add column progress_snapshot jsonb;

drop function if exists public.record_practice_result(
...
create or replace function public.record_practice_result(
  ...
  p_progress_snapshot jsonb default null
)
returns void
language plpgsql
security definer
...
begin
  ...
  insert into public.practice_results
    (user_id, wpm, raw_wpm, accuracy, mistakes, mode, duration, word_count, text_snapshot, progress_snapshot)
  values
    (v_user_id, p_wpm, p_raw_wpm, p_accuracy, p_mistakes, coalesce(p_mode, 'time'), p_duration, p_word_count,
     p_text_snapshot, p_progress_snapshot);
...
grant execute on function public.record_practice_result(
  numeric, numeric, numeric, integer, text, integer, integer, jsonb, text, jsonb
) to authenticated;

-- ============================================================
-- List a user's past runs suitable for ghost racing (own results only,
-- newest-first within their top few by speed for a given test config).
-- ============================================================
create or replace function public.list_ghost_candidates(
  p_mode text,
  p_duration integer default null,
  p_word_count integer default null,
...
stable
as $$
  select id, wpm, accuracy, duration, word_count, text_snapshot, progress_snapshot, created_at
  ...
  where user_id = auth.uid()
    ...
    and progress_snapshot is not null
  order by wpm desc
  limit p_limit;
$$;

grant execute on function public.list_ghost_candidates(text, integer, integer, integer) to authenticated;
