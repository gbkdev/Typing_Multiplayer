...
- `direct_messages` attachments (`0013_message_attachments.sql`) — a private `dm-attachments` Storage bucket plus `attachment_path`/`attachment_type`/`attachment_name` columns, so a DM can carry an image, gif, or generic file (10 MB limit) in addition to (or instead of) text. Access is scoped via a storage RLS policy that checks the sender/recipient UUIDs embedded in the object's folder path.
- Anti-cheat (`0014_anticheat.sql`) — `record_practice_result` and `submit_match_result` now validate WPM/accuracy are in plausible ranges and rate-limit solo submissions (1 per 2s per user), rejecting fabricated results before they ever reach the leaderboard.
- Weak-key practice (`0015_weak_keys.sql`) — `profiles.key_mistake_stats` (jsonb) tracks which characters you mistype most; the Profile stats tab surfaces your worst keys with a "Practice these" link into a biased drill (`/?practice=weak`).
- Ghost racing (`0016_ghost_racing.sql`) — every solo test now stores its exact text and a time-series of your progress (`text_snapshot`/`progress_snapshot` on `practice_results`). The typing page's ghost icon lets you pick a past run (via `list_ghost_candidates()`) and race it live, with a dual progress bar and a post-race "you beat your ghost by X wpm" comparison.
- Direct duel challenge — a "Duel" button in any DM thread creates a private 2-player room and sends the friend a special invite message (rendered as a "Join Duel" card, not plain text) that drops them straight into the room.
- Block & report (`0017_block_report.sql`) — new `blocks` and `reports` tables. Blocking someone immediately deletes any friendship (both directions) and is enforced in RLS so a blocked pair can't re-friend or DM each other. Manage blocks from Profile → Account settings; report from the "⋮" menu in a DM thread.
- Daily challenge (`0018_daily_challenge.sql`) — a `/daily` page where everyone races the identical text each calendar day (deterministically picked so it's the same for all players without a scheduled job), with its own mini-leaderboard. `get_daily_challenge()` lazily creates today's row on first request; `submit_daily_challenge_result()` keeps your best attempt.
